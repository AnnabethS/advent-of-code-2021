...
int main()
{
	...
	while(openListHead != NULL)
	{
		for(int i=0; i < LINES; i++)
		{
			...
			{ //capital
				/* printf("head (upper) %s\n", newNode); */
				visitSmall = openListHead->visitedSmall;
			}
			else if (newNode[0] >= 'a' && newNode[0] <= 'z')
			{ //lowercase
				/* printf("room %s, visitedSmall %d path %s\n", newNode, */
				/* 		openListHead->visitedSmall, openListHead->path); */
				if(isInPath(openListHead->path, newNode))
				{
					if(openListHead->visitedSmall)
					{
						continue;
					}
					else
			...
			openListTail->visitedSmall = visitSmall;
			char newPath[MAXPATHLENGTH];
			strcpy(newPath, openListHead->path);
			strcat(newPath, newNode);
			strcat(newPath, ",\0");
